...
class FailoverBreaker:
    def __init__(
        ...
        # Inert by default, like min_outage and notify_cooldown above and for the
        # same reason: this is POLICY, and the router supplies it from
        # Settings.failover_recovery_successes (2) for the Claude breaker only.
        #
        # It defaulted to 2 on the class and that reached further than intended.
        # The Codex breaker constructs without this argument, so it silently
        # inherited a two-success streak it was never designed for — and every
        # state-machine test that does failure-then-success stopped observing a
        # close, which is the mechanics those tests exist to check rather than
        # the policy layered on top.
    ...
    def record_success(self) -> bool:
        """Record one authenticated success; True when it completed a recovery.

        Returns a bool rather than None so the caller can release the claimed
        tiers on the exact call that closed the breaker, instead of sampling
        `open` before and after and inferring it.

        **The streak is anti-flap on the UPSTREAM signal.** One success can be
        luck on a link that is still failing, and closing on it puts the session
        straight back into the outage it just left, so `recovery_successes`
        consecutive successes are required.

        **It deliberately does not gate `maybe_recover`.** That path closes on a
        CONNECTIVITY probe, which answers a different question: it is the exact
        negation of what opened an offline-gated breaker ("this host is
        offline"), not evidence about whether the upstream is reliably back. A
        consecutive-upstream-successes counter is not measuring the same thing
        and does not compose with it. Two further reasons, both practical: the
        ticker already rations itself to one probe per `probe_interval`, so
        requiring two would put a two-interval floor on recovering from an
        outage that is already over — reintroducing the 77-minute stuck-open
        cost that ticker exists to remove — and the Codex breaker already gates
        that path on `_opened_on_transport`, so a streak would be a second,
        unrelated guard on one route back.
        """
        self._failures = 0
        if not self.open:
            self.reason = ""
        ...
        if self._recovery_streak < self.recovery_successes:
            logger.info(
                "%s recovery probe %d/%d succeeded; keeping failover open",
                self.upstream_name,
