...
- a bounded recall from claude-mem when `QWEN_MEMORY` is enabled;
- local Code Mode tools converted from Codex's Responses Lite namespace format.

It does not receive the old cloud transcript, cloud reasoning context, prompt-cache identifiers, OAuth headers, remote MCP schemas, hosted web-search tools, or image and file attachments. claude-mem provides continuity without forcing the 27B model to prefill the session that caused the outage or compaction failure. Backdoor reads that recall straight from the local replica at `~/.claude-mem/claude-mem.db`, read-only, so a host with no network still recalls. Durable fetched-source storage remains off unless an operator configures reviewed public URL prefixes; authenticated, browser-session, malformed, and unpaired tool results remain ephemeral. Set `QWEN_MEMORY=0` to suppress every memory read and write on the local path.
...
Recall on this path needs no credential: it reads the local claude-mem replica and writes through the worker on `127.0.0.1:37701`. Only the qwen wrapper's MCP shim needs `CMEM_PRO_TOKEN`, which it takes from the environment or `~/projects/.env`. No token is copied into the Backdoor LaunchAgent.
...
A short window is only workable if the facts have somewhere else to live. `QWEN_MEMORY` therefore defaults to **1** (flipped from opt-in on 2026-08-22, renamed from `QWEN_COGNEE` when Cognee was retired on 2026-09-04), attaching claude-mem over the two-tool stdio shim.

This is the one documented exception to the MCP-off rule, and the token arithmetic is why it survives that rule. The global MCP set costs about 142K tokens of schema. The shim exposes `memory_search` and `memory_remember` and nothing else, so it costs hundreds. Against a 32K window the first is impossible and the second is affordable.
...
Large fetched pages also bypass the model window at the proxy layer, making the behavior independent of GUI plugins. Once Claude, Codex, or another client returns a page as a tool result, Backdoor replaces results over 12,000 characters with up to 6,000 characters of passages ranked against the current question. Durable storage is off by default. `EXTERNAL_CONTEXT_PUBLIC_URL_PREFIXES` accepts comma-separated, reviewed public URL prefixes whose unauthenticated fetch output may be submitted with its URL and content hash to the local claude-mem worker on `127.0.0.1:37701`, which queues it durably and syncs it to the cmem.ai hub. Browser-session tools never persist their output. Later Qwen turns search that dataset and inject only a bounded set of recalled passages. The first turn does not wait for indexing: local ranking supplies its excerpts while the worker stores an approved durable copy in the background.

This boundary is client-independent because every local Qwen request crosses Backdoor. Backdoor does not fetch arbitrary URLs itself; the client remains responsible for browsing and authentication. Unapproved, authenticated, intranet, and client sources remain ephemeral even when their content looks harmless. Approved source text is marked as untrusted data, high-confidence credential-shaped documents are not written, and stored source URLs exclude user information, query strings, and fragments. Individual documents are capped at 500,000 characters before ranking, and each request can enqueue at most four documents. Recall opens the replica with a 1.5-second timeout, and Codex failover recall gives up after 2 seconds. A dead worker or a missing replica never drops the request. `QWEN_MEMORY=0` disables writes and recall for true-offline work, while the local 6,000-character reduction still protects Qwen's window.

The proxy resolves memory settings from its process environment and `profiles/<profile>.env`, and the qwen wrapper falls back to `CMEM_PRO_TOKEN` in `~/projects/.env`. That makes the same behavior available to terminal launchers and Dock-launched clients that never source `.zshrc`. A missing token or an unreachable worker degrades to local reduction only.
