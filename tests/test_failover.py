...
def make(clock, **kw):
    ...
    br = FailoverBreaker(
        ...
        # 1, matching the class default: these exercise breaker MECHANICS, and a
        # two-success streak is policy the router layers on top. Tests that are
        # about the streak pass recovery_successes=2 explicitly.
        recovery_successes=kw.pop("recovery_successes", 1),
        now_fn=clock,
...
def test_probe_success_closes_and_notifies():
    """Two successes, because this one is about the recovery streak itself."""
    clock = Clock()
    br, notes = make(clock, recovery_successes=2)
    for _ in range(3):
        br.record_failure("ConnectError")
    assert br.open
...
def test_an_oversized_session_gets_no_failover_tier_by_design():
    """This branch changes the contract, and that is the point.

    `main` asserted `pick_failover_profile(10**9) is not None` — an outage must
    never leave a session with no tier. That guarantee was right for a ladder
    ending in an unbounded 4B: something always answers, even if badly.

    Context virtualization removes the premise. The prompt is reduced to fit
    BEFORE the selector runs, so a request that still exceeds every tier is not
    a large session needing a wider model — it is a request no local tier can
    honestly serve. Returning None lets the caller give a continuity response
    instead of a model call that cannot complete. `main`'s own comments record
    what the alternative cost: a 143,490-token session hitting a 32K window 87
    times over ~17 hours, loading 23GB on each attempt.

    The route ladder keeps its wide escape hatch, because a deliberate
    `/model qwen` is not an outage and the user asked for that tier by name.
    """
    assert pick_failover_profile(10**9) is None
    assert pick_route_profile(240_000) == "local-failover-256k"
